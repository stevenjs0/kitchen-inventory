...
export function LocationTree({ locations, onLocationSelect }: LocationTreeProps) {
  const groupedBySection = locations.reduce((acc, loc) => {
    if (!acc[loc.section]) {
      acc[loc.section] = [];
    }
            ...
            {sectionLocations.map((location) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <div className="font-semibold text-sm group-hover:text-primary transition-colors">{location.name}</div>
                </div>
                <div className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider flex items-center gap-2">
                  ...
                  <span className="px-1.5 py-0.5 rounded bg-muted text-primary ml-auto">{location.level}</span>
                </div>
              </button>
            ))}
          </div>
